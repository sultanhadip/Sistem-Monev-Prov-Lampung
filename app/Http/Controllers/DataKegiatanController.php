...
class DataKegiatanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $filterTim = $request->input('filterTim');
        ...
        $datakegiatan = $query->paginate($recordsPerPage, ['*'], 'page', $page);

        $showingText = 'Showing ' . $datakegiatan->firstItem() . '-' . $datakegiatan->lastItem() . ' of ' . $datakegiatan->total() . ' records';

        // Jika permintaan adalah Ajax, kembalikan hanya bagian tabel dan showingText
        if ($request->ajax()) {
            $tableHtml = view('datakegiatan.table', [
                'datakegiatan' => $datakegiatan,
                'timkerja'     => $timkerja      // <— tambahkan
            ])->render();

            $paginationHtml = view('datakegiatan.pagination', compact('datakegiatan'))->render();
        ...
        return view('data-kegiatan', compact('datakegiatan', 'timkerja', 'showingText', 'filterTim'));
    }
