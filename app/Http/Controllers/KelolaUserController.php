...
class KelolaUserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $satuanKerjaId = $request->input('filterSatuanKerja');
        $timKerjaId = $request->input('filterTimKerja');
        ...
        // Admin Satuan Kerja filter
        if ($isAdminSatker) {
            $userSatuanKerjaId = $currentUser->kode_satuan_kerja; // Get current user's Satuan Kerja ID
            $query->where('kode_satuan_kerja', $userSatuanKerjaId); // Filter by Satuan Kerja
        }

        // Kondisi untuk Admin Provinsi
        if ($isAdminProvinsi) {
            $userTimKerjaId = $currentUser->kode_tim;  // Menambahkan filter berdasarkan tim kerja user
            $query->where('kode_tim', $userTimKerjaId);
        }
        ...
        // Batasi satuan kerja pada dropdown jika admin satker
        if ($isAdminSatker) {
        ...
        // Tim kerja dapat difilter hanya jika admin provinsi
        if ($isAdminProvinsi) {
            $timKerja = TimKerja::where('id', $currentUser->kode_tim)->get();
        } else {
