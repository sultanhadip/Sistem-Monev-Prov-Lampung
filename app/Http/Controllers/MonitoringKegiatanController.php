...
class MonitoringKegiatanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $qb = MonitoringKegiatan::with(['timkerja', 'datakegiatan', 'targetRealisasiSatker'])
            ...
            ->when($filterTahun, function ($q) use ($filterTahun) {
                $q->whereYear('waktu_mulai', $filterTahun)
                    ->orWhereYear('waktu_selesai', $filterTahun);  // Menambahkan kondisi untuk tahun pada waktu_selesai
            })
            ->when($filterBulan, function ($q) use ($filterBulan) {
                $q->where(function ($query) use ($filterBulan) {
                    $query->whereMonth('waktu_mulai', $filterBulan)
                        ->orWhereMonth('waktu_selesai', $filterBulan)  // Menambahkan kondisi untuk bulan pada waktu_selesai
                        ->orWhere(function ($query) use ($filterBulan) {
                            $query->whereMonth('waktu_mulai', '<', $filterBulan) // Kegiatan yang dimulai sebelum bulan filter dan berakhir di bulan filter
                                ->whereMonth('waktu_selesai', '>=', $filterBulan);
                        });
                });
            });
        ...
        foreach ($monitoringKegiatan as $item) {
            ...
            $item->waktu_selesai  = $selesai->format('Y-m-d');
            ...
            // Pastikan bahwa target_sampel kosong jika tidak ada data target
            if ($trs->isNotEmpty()) {
                $item->target_sampel = $trs->pluck('target_satker', 'kode_satuan_kerja')->toArray();
            } else {
                $item->target_sampel = []; // Kosongkan jika tidak ada target
            }
            ...
            // Gunakan isoFormat() untuk mendapatkan nama bulan dalam bahasa Indonesia
            $dt = Carbon::parse($item->waktu_mulai);
            $item->periode_kegiatan = $dt->isoFormat('MMMM YYYY');
            $mIdx = $dt->month - 1;
            $yr = $dt->year;
