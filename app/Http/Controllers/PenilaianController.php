...
class PenilaianController extends Controller
{
        ...
        $qb = Penilaian::with('satuanKerja')
            ...
            ->when($filterBulan, function ($q) use ($filterBulan) {
                ...
                $monthName = \Carbon\Carbon::createFromDate(null, $m, 1)
                    ->locale('id')
                    ->isoFormat('MMMM'); // Menampilkan nama bulan dalam format panjang
    ...
    public function generate(Request $request)
    {
        try {
            ...
            $bulanNama = now()->monthName;
            $bulanNamaIndo = $this->getNamaBulanIndo($bulanNama);
            $currentPeriod = $bulanNamaIndo . ' ' . $currentYear;
            ...
            // Step 2: Menghitung nilai rata-rata tim kerja per satuan kerja
            $nilaiSatuanKerja = [];
            ...
            // Ambil data penilaian yang terbaru untuk diperbarui
            $penilaian = Penilaian::with('satuanKerja')
                ->orderBy('kode_satuan_kerja', 'asc')
                ->get();

            // Kirim data terbaru sebagai HTML atau JSON
            $view = view('penilaian.table', compact('penilaian'))->render();  // Mengambil bagian tabel sebagai HTML

            return response()->json([
                'message' => 'Penilaian berhasil digenerate dan dirangking untuk periode ini.',
                'tableHtml' => $view  // Mengembalikan HTML tabel yang baru
            ]);
        } catch (\Exception $e) {
            Log::error('Error generating performance review: ' . $e->getMessage());
            return response()->json(['message' => 'Terjadi kesalahan saat generate penilaian.'], 500);
        }
    }
