...
class SatuanKerjaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $satuankerja = $query->paginate($recordsPerPage, ['*'], 'page', $page);

        $showingText = 'Showing ' . $satuankerja->firstItem() . '-' . $satuankerja->lastItem() . ' of ' . $satuankerja->total() . ' records';

        // Jika permintaan adalah Ajax, kembalikan hanya bagian tabel dan showingText
        if ($request->ajax()) {
            $tableHtml = view('satuankerja.table', [
                'satuankerja'     => $satuankerja
            ])->render();

            $paginationHtml = view('satuankerja.pagination', compact('satuankerja'))->render();
        ...
        return view('satuan-kerja', compact('satuankerja', 'showingText'));
    }
