...
<body>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <table class="table table-hover table-bordered text-center mb-4" id="dataTable">
                ...
                <tbody>
                  @include('datakegiatan.table', ['datakegiatan' => $datakegiatan, 'timkerja' => $timkerja])
                </tbody>
              </table>
              ...
              <div class="d-flex justify-content-between align-items-center">
                <!-- Showing Text -->
                <div class="me-3">
                  <span id="showingText">Showing {{ $datakegiatan->firstItem() }}-{{ $datakegiatan->lastItem() }} of {{ $datakegiatan->total() }} records</span>
                </div>
                ...
                <div class="d-flex align-items-center ms-auto">
                  ...
                  <div id="paginationContainer">
                    @include('datakegiatan.pagination', ['datakegiatan' => $datakegiatan])
                  </div>
                </div>
              </div>
              ...
              <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content rounded-1">
                    <form action="{{ route('datakegiatan.update', ':id') }}" method="POST">
                      @csrf
                      @method('PUT')
                      <div class="modal-header">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="mb-4">
                          <label for="periode_kegiatan" class="form-label mb-0">Periode Kegiatan</label>
                          <select class="form-select mt-1" id="periode_kegiatan" name="periode_kegiatan" required>
                            <option value="">Pilih Periode Kegiatan</option>
                            <option value="Bulanan">Bulanan</option>
                            <option value="Triwulan">Triwulan</option>
                            <option value="Semesteran">Semesteran</option>
  ...
  <script>
    $(document).ready(function() {
      ...
      document.getElementById("filterTim").addEventListener("change", function() {
        applyFilters(1);
      });
  ...
  <script>
    $('#editModal').on('show.bs.modal', function(event) {
      ...
      var objek_kegiatan = button.data('objek_kegiatan');
      var periode_kegiatan = button.data('periode_kegiatan');
      var kode_tim = button.data('kode_tim');

      // Set form action URL
      var formAction = '{{ route("datakegiatan.update", ":id") }}';
      formAction = formAction.replace(':id', id);
      modal.find('form').attr('action', formAction);

      // Isi field dengan data yang dipilih
      modal.find('#nama_kegiatan').val(nama_kegiatan);
      modal.find('#objek_kegiatan').val(objek_kegiatan);
      modal.find('#periode_kegiatan').val(periode_kegiatan);
      modal.find('#kode_tim').val(kode_tim);
    });
  </script>

  <script>
    // Ketika modal konfirmasi muncul
    $('#editModal').on('show.bs.modal', function(event) {
      var button = $(event.relatedTarget); // Tombol yang memicu modal
      var url = button.data('url'); // Mengambil URL untuk penghapusan
      var modal = $(this);

      // Setel action form untuk menghapus data
