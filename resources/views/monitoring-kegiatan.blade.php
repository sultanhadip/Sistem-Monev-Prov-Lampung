...
<body>
            ...
            <div class="card-body">
              ...
              <div id="filterForm" class="row g-2 align-items-center mb-3">
                ...
                <div class="col-md-3">
                  <select
                    ...
                    <option value="{{ $m }}" {{ (int)request('filter_bulan') === $m ? 'selected' : '' }}>
              ...
              <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content rounded-1">
                    <!-- Formulir Edit Modal -->
                    <form action="{{ route('monitoringkegiatan.update', ':id') }}" method="POST">
                      @csrf
                      @method('PUT')
                      ...
                      <div class="modal-body">
                        ...
                        <!-- Alokasi Target Sampel -->
                        <div class="mb-4">
                          <label for="target_sampel" class="form-label mb-0">Alokasi Target Sampel</label>
                          <div class="mt-3">
                            @foreach ($satuankerja as $satker)
                            <div class="row mb-2">
                              <label for="target_sampel_{{ $satker->id }}" class="col-sm-4 col-form-label" style="font-size: 11px;">
                                [{{ $satker->kode_satuan_kerja }}] {{ $satker->nama_satuan_kerja }}
                              </label>
                              <div class="col-sm-8">
                                <input type="number" class="form-control" name="target_sampel[{{ $satker->id }}]" id="target_sampel_{{ $satker->id }}" value="{{ old('target_sampel.' . $satker->id, $satker->target_sampel) }}" style="font-size: 12px;">
                              </div>
                            </div>
                            @endforeach
                          </div>
  ...
  <script>
    $('#editModal').on('show.bs.modal', function(event) {
      ...
      var waktu_mulai = button.data('waktu_mulai');
      var waktu_selesai = button.data('waktu_selesai');
      var target_sampel = button.data('target_sampel');

      // Set form action URL
      var formAction = '{{ route("monitoringkegiatan.update", ":id") }}';
      formAction = formAction.replace(':id', id);
      modal.find('form').attr('action', formAction);

      // Isi field dengan data yang dipilih
      modal.find('#kode_tim').val(kode_tim);
      modal.find('#kode_kegiatan').val(kode_kegiatan);
      modal.find('#waktu_mulai').val(waktu_mulai);
      modal.find('#waktu_selesai').val(waktu_selesai);

      // Reset target_sampel
      modal.find('input[name^="target_sampel"]').val('');

      // Jika ada data target_sampel, masukkan nilainya ke dalam input
      if (target_sampel) {
        $.each(target_sampel, function(kode_satuan_kerja, target) {
          var targetInput = modal.find('input[name="target_sampel[' + kode_satuan_kerja + ']"]');
