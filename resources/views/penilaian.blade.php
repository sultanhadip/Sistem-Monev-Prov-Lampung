...
<body>
  ...
  <script>
        ...
        if (confirm("Apakah Anda yakin ingin mengenerate nilai kinerja?")) {
          $.ajax({
            ...
            success: function(response) {
              ...
              updateTable(response.tableHtml);
            },
            error: function(xhr) {
              let errorMessage = xhr.responseJSON ? xhr.responseJSON.message : "Terjadi kesalahan saat generate.";
    ...
    `);
        ...
        setTimeout(() => {
          const alertNode = alertContainer.find('.alert');
          if (alertNode) {
      ...
      function updateTable(newTableHtml) {
        $("#dataTable tbody").html(newTableHtml);
      }
    });
  </script>
